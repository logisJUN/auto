"""trade_management.range_trade.min_atr_pct: range/scalp entries use a
tighter SL/TP multiplier than trend trades (more fee-sensitive per trade), so
they require a stricter minimum volatility floor than the general
risk.min_volatility_atr_pct before even being considered.
"""
...
RANGE_CFG = {
    "enabled": True, "edge_atr_mult": 0.5, "max_range_width_atr_mult": 4.0,
    "atr_sl_multiplier": 1.3, "atr_tp_multiplier": 1.3, "min_atr_pct": 0.25,
}
...
RAW_CFG = {
    ...
    "risk": {
        ...
        "min_volatility_atr_pct": 0.0,  # disabled -- isolate the range-specific floor
    },
    "signals": {
...
def _make_strategy(tmp_path, range_cfg=RANGE_CFG):
    secrets = Secrets(bybit_api_key="x", bybit_api_secret="y", bybit_testnet=True,
                       ...
                       dashboard_token="t")
    raw = {**RAW_CFG, "trade_management": {**RAW_CFG["trade_management"], "range_trade": range_cfg}}
    ...
    client.get_position.return_value = {
        ...
        "unrealized_pnl": 0.0, "position_idx": 0, "stop_loss": 99.0, "take_profit": 101.0,
    }
    return Strategy(client, cfg, state, notifier, str(tmp_path / "logs")), state, client


def _signal(atr, confidence=0.0, direction="neutral"):
    range_low = 100.0
    range_high = range_low + atr * 2.0  # comfortably under max_range_width_atr_mult(4.0)*atr
    return {"score": 0.0, "confidence": confidence, "direction": direction,
            "atr": atr, "close": 100.0, "range_high": range_high, "range_low": range_low, "components": {}}


def test_range_entry_skipped_below_range_specific_floor(tmp_path):
    strategy, state, client = _make_strategy(tmp_path)
    strategy.get_signal = lambda s, force=False: _signal(atr=0.2)  # 0.2% < min_atr_pct(0.25%)
...
def test_range_entry_allowed_above_range_specific_floor(tmp_path):
    strategy, state, client = _make_strategy(tmp_path)
    strategy.get_signal = lambda s, force=False: _signal(atr=0.3)  # 0.3% >= min_atr_pct(0.25%)
...
def test_range_floor_disabled_when_min_atr_pct_is_zero(tmp_path):
    cfg = {**RANGE_CFG, "min_atr_pct": 0.0}
    strategy, state, client = _make_strategy(tmp_path, range_cfg=cfg)
    strategy.get_signal = lambda s, force=False: _signal(atr=0.05)  # would fail the 0.25% floor
...
def test_range_sl_tp_use_the_widened_multiplier(tmp_path):
    strategy, state, client = _make_strategy(tmp_path)
    ...
    # long entry at 100, atr=1.0, atr_sl_multiplier=1.3 -> SL = 100 - 1.3 = 98.7
    assert trade["initial_sl"] == 98.7
    assert trade["initial_tp"] == 101.3
